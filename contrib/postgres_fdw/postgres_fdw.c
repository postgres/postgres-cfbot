						   ...
						   MemoryContext temp_context)
{
	 ...
	 *
	 * Similarly, reconstruct any plain whole-row reference to this same
	 * relation that deparseExplicitTargetList() also skipped, since its
	 * columns are already being fetched individually elsewhere in this same
	 * tlist: build it here from the values already decoded above, instead of
	 * having the remote side ship a second, redundant ROW(...) for it.
	 */
	if (fsstate)
	{
		...
		if (fsplan->scan.scanrelid > 0 && fsplan->fdw_scan_tlist != NIL)
		{
			...
			if (wholerow_pos >= 0)
			{
				...
				Assert(relvalues != NULL);
				rowtup = heap_form_tuple(reltupdesc, relvalues, relnulls);
				values[wholerow_pos - 1] = heap_copy_tuple_as_datum(rowtup, reltupdesc);
