...
-- REMOTE_PLANS cannot be combined with ANALYZE
...
-- An INSERT whose target list contains only generated columns has no
-- placeholders, despite having a nonempty targetAttrs list.
CREATE TABLE generated_tbl (a int GENERATED ALWAYS AS (1) STORED);
CREATE FOREIGN TABLE generated_ft (a int GENERATED ALWAYS AS (1) STORED)
  SERVER loopback OPTIONS (table_name 'generated_tbl');
EXPLAIN (REMOTE_PLANS, VERBOSE, COSTS OFF)
INSERT INTO generated_ft DEFAULT VALUES;
...
DROP FOREIGN TABLE generated_ft;
DROP TABLE base_tbl3;
DROP TABLE base_tbl4;
DROP TABLE generated_tbl;
