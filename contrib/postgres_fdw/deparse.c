...
/*
 * Does tlist contain an individual real column (varattno > 0, excluding the
 * RemoteTableOidAttributeNumber pseudo-column) of relid, besides a whole-row
 * entry for it?  Used to tell whether a whole-row reference has anything to
 * be locally reconstructed from (see deparseExplicitTargetList()).
 */
static bool
 ...
 *   from.  There can be more than one such whole-row entry for the same
 *   relation (e.g. one from the query itself, another added by core code
 *   for row identity); each is skipped and reconstructed independently.
 *
 * This is used for both SELECT and RETURNING targetlists; the is_returning
 * parameter is true only for a RETURNING targetlist.
 */
