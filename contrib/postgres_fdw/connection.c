...
GetConnection(UserMapping *user, bool will_prep_stmt, PgFdwConnState **state)
{
	...
	PG_TRY();
	{
		...
		/*
		 * If some other scan on this connection is using streaming_fetch and
		 * still has an unconsumed result, drain it too, so the caller gets an
		 * idle connection back.
		 */
		drain_other_active_scan(&entry->state);
 ...
 * so that if some other streaming_fetch scan on this connection still has
...
pgfdw_exec_query(PGconn *conn, const char *query, PgFdwConnState *state)
{
	...
	/*
	 * If some other scan on this connection is using streaming_fetch and
	 * still has an unconsumed result, drain it now so the connection is idle
	 * before we send a new query.
	 */
	drain_other_active_scan(state);
