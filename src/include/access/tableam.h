...
typedef struct TableAmRoutine
{
	 ...
	 *
	 * This is a low-level callback for single-shot TID lookups used by
 ...
 * low-level interface designed for use by constraint enforcement code, where
 * passing a TID can't be avoided.
 *
 ...
 * function is that we test the currently visible version of a row, which
 * matters with AMs that support storing multiple row versions reachable via a
 * single TID (e.g., due to heapam's HOT chains).  To reliably fetch exactly
 * the tuple at `tid`, call table_tuple_fetch_row_version() instead.
 */
static inline bool
