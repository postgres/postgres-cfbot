								 ...
								 bool throttle)
{
	...
	/*
	 * Ran out of items from prefetchBatch.  Try to advance to the scan's next
	 * batch.
	 */
	if (unlikely(index_scan_batch_full(scan)) || unlikely(throttle))
	{
		 ...
		 * index_scan_pos_advance just made to prefetchPos before returning,
		 * leaving it in a state that's consistent with the work actually
		 * performed (various positional state assertions expect this).
		 */
		if (ScanDirectionIsForward(direction))
		{
