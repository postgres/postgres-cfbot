...
typedef struct SafeTypeCastExpr
{
	...
	/*
	 * The transformed input expression being cast, evaluated once and fed to
	 * castexpr
	 */
	Expr	   *arg;

	/*
	 * coercion of arg to the target type, built over a CaseTestExpr that
	 * stands for arg's value; NULL means no cast path exists, in which case
	 * the result is always defexpr
	 */
	Expr	   *castexpr;
