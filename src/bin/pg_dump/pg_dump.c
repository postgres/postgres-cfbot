...
getDependencies(Archive *fout)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/*
		 * An extension with an owned schema depends on its schema, but that
		 * schema is also a member of the extension, so keeping both
		 * dependencies would create a loop.  We never actually dump a CREATE
		 * SCHEMA command for an owned schema anyway, so the dependency of the
		 * extension on the schema can be ignored.  We still keep the schema's
		 * dependency on the extension so the schema's ACL and comment are
		 * dumped after CREATE EXTENSION creates the schema.
		 */
		if (dobj->objType == DO_EXTENSION &&
			refdobj->objType == DO_NAMESPACE &&
			((ExtensionInfo *) dobj)->ownedschema)
			continue;
