				   ...
				   int location)
{
	...
	/*
	 * default expression must be a constant - it cannot contain a reference
	 * to a column of the aggregate's own query level (a Var of the current
	 * level would vary across the very rows being aggregated), nor an
	 * aggregate or window function.
	 *
	 * A reference to an outer query's column (varlevelsup > 0) is fine: it
	 * is fixed for the duration of evaluating this query, exactly like a
	 * Param or a stable function call, which are already allowed.  We
	 * cannot use contain_var_clause() to make this distinction, as it does
	 * not look at varlevelsup at all; use a dedicated walker instead.
	 */
 ...
 *		Returns true if the node tree contains a Var of the current query
 *		level, or a CurrentOfExpr.
 *
 * A Var of an outer query level (varlevelsup > 0) is not reported: it is
 * constant for the duration of evaluating the current query, just like a
 * Param.  CurrentOfExpr is always tied to the current scan, so it is
 * rejected unconditionally.  Sub-selects are rejected separately (their
 * Query is not descended into by expression_tree_walker anyway).  This runs
 * during parse analysis, so PlaceHolderVars (which the planner introduces)
 * cannot appear; a Var inside one would be caught by the recursion anyway.
 */
static bool
