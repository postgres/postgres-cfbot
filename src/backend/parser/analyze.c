 ...
 * ordinary multi-row VALUES INSERT with an explicit column list), the
 * indirection expression built for a partial (subfield/subscript) target is
 * applied uniformly to every row.  There is therefore no way for one row's
 * slot to mean "update this element" while another row's slot for the same
 * shared target means "leave the column alone" (target omitted) or "replace
 * the whole column" (whole-column target): both would have to silently
 * become an illegal "set an element/subfield to DEFAULT", or would only be
 * caught downstream as a confusing "column specified more than once".  To
 * avoid that, if a partial target is used at all, every row must supply
 * that exact same target (same column and indirection); otherwise we raise
 * a clear error here.  Rows that all agree on the same indirection target
 * are unaffected and continue to work, just like the equivalent multi-row
 * "INSERT INTO t (id, arr[1]) VALUES ..." form.
 */
static void
						 ...
						 List **cols_p, List **valuesLists_p)
{
	...
	/*
	 * Second pass (validation): if a partial (subfield/subscript) target is
	 * used anywhere and there's more than one row, require every row to
	 * supply that exact same target.  See the function comment above for
	 * why this can't be relaxed while still flattening all rows into one
	 * shared column list.
	 */
	if (list_length(setClause) > 1)
	{
		foreach_node(ResTarget, col, all_cols)
		{
			...
			foreach_node(List, set_clause, setClause)
			{
				...
				if (!row_has_target)
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							 errmsg("array/field indirection target \"%s\" must be specified identically in every row of a multi-row INSERT ... SET",
									col->name),
							 errdetail("One or more rows omit this target, assign to the whole column instead, or use different indirection."),
	...
	/*
	 * Third pass: for each row, create a values list matching the target
	...
	foreach_node(List, set_clause, setClause)
	{
		...
		foreach_node(ResTarget, col, all_cols)
		{
			...
			else
			{
				 ...
				 * DEFAULT.  This can only happen for a whole-column target
				 * here: the validation pass above already guarantees that
				 * any partial (indirection) target is present in every
				 * row, so we'll never generate an illegal "set an
				 * element/subfield to DEFAULT" from this fallback.
				 */
