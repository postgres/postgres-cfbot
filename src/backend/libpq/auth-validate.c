...
EnableCredentialValidationTimeout(void)
{
	...
	/*
	 * Skip for replication connections.  Once a walsender enters its own
	 * streaming loop it never returns to the command dispatch loop where
	 * CredentialValidationTimeoutPending is consumed, so arming this timer
	 * for it would just leave a pending flag that is never acted on.
	 * Replication connections are out of scope for this feature.
	 */
...
CheckCredentialValidity(void)
{
	 ...
	 * - replication connections (walsenders never return to the command
	 *   dispatch loop that consumes CredentialValidationTimeoutPending once
	 *   they start streaming, so they are out of scope for this feature)
	 * - while authentication is still in progress
	 *
	 * Note: we do NOT skip validation during RecoveryInProgress() -- a
	 * hot-standby server accepts ordinary read-only client sessions, and
	 * those need the same periodic re-validation as on a primary.
	 */
