 ...
 *
 * Checks pg_authid.rolvaliduntil for the authenticated role; this is
 * role-level and auth-method-independent, so it governs password,
 * certificate, OAuth, etc. sessions alike.  Also treats a role that no
 * longer exists as invalid.
 *
 * Uses GetAuthenticatedUserId(), not GetSessionUserId(), so that SET SESSION
 * AUTHORIZATION can't mask the original login role's expiry or removal.
 *
