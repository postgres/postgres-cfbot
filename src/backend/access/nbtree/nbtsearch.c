...
static inline void _bt_batchscan_unlock(IndexScanDesc scan, IndexScanBatch batch,
										Buffer buf);
...
/*
 * _bt_batchscan_unlock() -- nbtree wrapper for batchscan_unlock.
 *
 * Performs the same Valgrind instrumentation as _bt_unlockbuf.
 */
static inline void
_bt_batchscan_unlock(IndexScanDesc scan, IndexScanBatch batch, Buffer buf)
{
	...
	batchscan_unlock(scan, batch, buf);
...
_bt_next(IndexScanDesc scan, ScanDirection dir, IndexScanBatch priorbatch)
{
	...
	lastcurrblkno = btpriorbatch->batchPage;
				  ...
				  OffsetNumber offnum, ScanDirection dir)
{
	...
	if (_bt_readpage(scan, firstbatch, dir, offnum, true))
	{
		...
		_bt_batchscan_unlock(scan, firstbatch, btfirstbatch->buf);
		return firstbatch;
	}
	...
	lastcurrblkno = btfirstbatch->batchPage;
	moreInDir = ScanDirectionIsForward(dir) ?
 ...
 * current scan direction), and lastcurrblkno is the prior batch's leaf page.
				 ...
				 BlockNumber lastcurrblkno, ScanDirection dir, bool firstpage)
{
	...
	Assert(btnewbatch->batchPage == blkno);
	_bt_batchscan_unlock(scan, newbatch, btnewbatch->buf);
...
_bt_endpoint(IndexScanDesc scan, ScanDirection dir)
{
	...
	if (!BufferIsValid(btfirstbatch->buf))
	{
		...
		{
			PredicateLockRelation(rel, scan->xs_snapshot);
			btfirstbatch->buf = _bt_get_endpoint(rel, 0,
												 ScanDirectionIsBackward(dir));
		}
