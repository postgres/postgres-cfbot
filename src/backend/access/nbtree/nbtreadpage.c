...
typedef struct BTReadPageState
{
	...
	BlockNumber batchpage;		/* leaf page being read */
			 ...
			 OffsetNumber offnum, bool firstpage)
{
	...
	pstate.batchpage = btnewbatch->batchPage = BufferGetBlockNumber(btnewbatch->buf);
	...
	if (scan->parallel_scan)
	{
		...
		if (ScanDirectionIsForward(dir))
			_bt_parallel_release(scan, btnewbatch->nextPage,
								 btnewbatch->batchPage);
		else
			_bt_parallel_release(scan, btnewbatch->prevPage,
								 btnewbatch->batchPage);
	}

	PredicateLockPage(rel, pstate.batchpage, scan->xs_snapshot);
			 ...
			 IndexTuple itup, int *tupleOffset)
{
	...
	if (newbatch->tuples)
	{
		...
		memcpy(newbatch->tuples + *tupleOffset, itup, itupsz);
					  ...
					  IndexTuple itup, int *tupleOffset)
{
	...
	if (newbatch->tuples)
	{
		...
		base = (IndexTuple) (newbatch->tuples + *tupleOffset);
...
new_prim_scan:
	...
	if (scan->parallel_scan)
		_bt_parallel_primscan_schedule(scan, pstate->batchpage);
