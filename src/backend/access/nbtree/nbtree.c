...
btgetbatch(IndexScanDesc scan, IndexScanBatch priorbatch, ScanDirection dir)
{
	...
	/*
	 * When the just-read leaf page already ended the scan in a direction,
	 * mark the batch accordingly
	 */
	if (batch && !so->needPrimScan)
	{
		BTBatchData *btbatch = BTBatchGetData(scan, batch);

		if (!btbatch->moreRight)
...
btunguardbatch(IndexScanDesc scan, IndexScanBatch batch)
{
	...
	Assert(BufferGetBlockNumber(btbatch->buf) == btbatch->batchPage);
...
btkillitemsbatch(IndexScanDesc scan, IndexScanBatch batch)
{
	...
	buf = _bt_getbuf(rel, btbatch->batchPage, BT_READ);
