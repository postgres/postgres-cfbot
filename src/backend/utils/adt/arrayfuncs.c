...
generate_subscripts_support(PG_FUNCTION_ARGS)
{
	...
	if (IsA(rawreq, SupportRequestRows))
	{
		...
		/*
		 * Be paranoid: only proceed when we are attached to one of the
		 * generate_subscripts() entries, the call site is a FuncExpr carrying
		 * enough arguments, and root is available, since
		 * estimate_expression_value() requires it.  Otherwise decline, and
		 * prorows applies.
		 */
		if ((req->funcid == F_GENERATE_SUBSCRIPTS_ANYARRAY_INT4 ||
			 req->funcid == F_GENERATE_SUBSCRIPTS_ANYARRAY_INT4_BOOL) &&
			req->root != NULL &&
			is_funcclause(req->node) &&
			list_length(((FuncExpr *) req->node)->args) >= 2)
		{
			List	   *args = ((FuncExpr *) req->node)->args;

			/* The parser enforces the signature for these two functions */
			Assert(list_length(args) == 2 || list_length(args) == 3);
			Assert(exprType(lsecond(args)) == INT4OID);

			/* We can use estimated argument values here */
			Node	   *arg1 = estimate_expression_value(req->root,
														 linitial(args));
			Node	   *arg2 = estimate_expression_value(req->root,
														 lsecond(args));
			Node	   *arg3 = NULL;

			/* The third argument, "reverse", is optional */
