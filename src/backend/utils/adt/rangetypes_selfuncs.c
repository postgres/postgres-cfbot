...
rangejoinsel(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Can't use the histograms with insecure range support functions.  The
	 * estimator compares bounds taken from both sides' histograms with the
	 * range comparison function, and interpolation may additionally invoke
	 * the subtype difference function, so neither function may see the
	 * statistics unless that is safe for both relations.
	 */
