 ...
 * HIST_IO_NUM_COLUMNS.
 */
typedef enum histogram_io_stat_col
{
 ...
 *
 * Helper routine for pg_stat_get_io_histogram() filling a result tuplestore
 * with one tuple per latency bucket for each object, context and I/O
 * operation tracked for the given backend type, based on the contents of
 * bktype_stats and the histogram slots in backends_io_stats.
 */
static void
...
pg_stat_get_io_histogram(PG_FUNCTION_ARGS)
{
	...
	for (int bktype = 0; bktype < BACKEND_NUM_TYPES; bktype++)
	{
		...
		/*
		 * In Assert builds, we can afford an extra loop through all of the
		 * counters (in pgstat_bktype_io_stats_valid()), checking that only
		 * expected stats are non-zero, since it keeps the non-Assert code
		 * cleaner.
		 */
		Assert(pgstat_bktype_io_stats_valid(bktype_stats, bktype));
