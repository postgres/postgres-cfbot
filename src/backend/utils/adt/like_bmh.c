...
/*
 * A Param can look stable, but PL/pgSQL simple expressions reuse one
 * ExprState while the variable changes.  Cache only Const patterns without
 * revalidation; other patterns use the revalidation path.  A
 * ScalarArrayOpExpr is also rejected: its array argument can be a Const while
 * the element passed to this function changes between calls.
 */
static bool
...
like_bmh_init(const char *p, int plen, FmgrInfo *flinfo, Oid collation)
{
	...
	pattern_stable = like_bmh_pattern_arg_is_const(flinfo);
