...
write_syslog(int level, const char *line)
{
	...
	{
		/*
		 * With LOG_NDELAY, openlog() connects to the system logger right
		 * here rather than on the first message, and that can block (for
		 * example glibc's stream-socket fallback against a busy syslog
		 * daemon), so report the wait.  Backends normally inherit the
		 * connection from the postmaster and never get here; they do after
		 * syslog_ident or syslog_facility changes, since
		 * assign_syslog_facility() closes the connection in every process
		 * that applies the reload.  A nested log write briefly masks any
		 * outer wait event (wait events are single-slot).
		 */
