 ...
 *
 * Function stats are not transactional, so this always returns
 * PGSTAT_FLUSH_DONE.  The entry may be flushed more than once per transaction;
 * see PgStat_FunctionStatus for the counts/flushed delta scheme.
 */
PgStat_FlushResult
						 ...
						 bool xact_boundary)
{
	...
	/*
	 * Nothing new since the last flush; skip without taking the lock.  A byte
	 * compare is safe as PgStat_FunctionCounts holds only int64 fields.
	 */
	...
	shfuncent->stats.self_time +=
		...
		INSTR_TIME_GET_MICROSEC(localent->flushed.self_time);

	pgstat_unlock_entry(entry_ref);

	/* Record the new baseline while in a transaction and still accumulating. */
	if (!xact_boundary)
