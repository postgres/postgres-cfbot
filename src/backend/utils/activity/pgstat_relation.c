...
/*
 * Report that the table was skipped during vacuum and/or analyze.
 */
void
pgstat_report_skipped_vacuum_analyze(Oid relid, int flags)
{
	...
	if (relkind != RELKIND_RELATION &&
		...
		flags &= ~(PGSTAT_REPORT_LOCK_SKIPPED_VACUUM |
				   PGSTAT_REPORT_LOCK_SKIPPED_AUTOVACUUM);

	if (relkind != RELKIND_RELATION &&
		relkind != RELKIND_MATVIEW &&
		relkind != RELKIND_PARTITIONED_TABLE)
		flags &= ~(PGSTAT_REPORT_LOCK_SKIPPED_ANALYZE |
				   PGSTAT_REPORT_LOCK_SKIPPED_AUTOANALYZE);
	 ...
	 * after our earlier check is visible to the syscache lookup below.  This
	 * has to happen after releasing the stats entry lock, because resolving a
	 * catcache miss may need to access pg_class and wait for a heavyweight
	 * lock while still holding that LWLock, which risks deadlock.
	 */
