 ...
 * Block I/O times cannot: pgstat_count_io_op_time_ext() also counts them in
 * the checkpointer and the background writer, which never call that function.
 * A PgStat_EntryRef->pending entry created there would never be flushed, and
 ...
 * process-local memory, and flushed through flush_static_cb and whenever the
 * I/O statistics are flushed by pgstat_flush_io().
 *
...
/*
 * The entry used last.  I/O tends to come in runs against the same
 * tablespace, so this saves a hash lookup almost every time.
 */
static PgStat_PendingTabspaceTime *last_tabspace_time = NULL;
...
{
	PgStat_PendingTabspaceTime *pending = last_tabspace_time;

	Assert(OidIsValid(spcoid));

	if (pending == NULL || pending->spcoid != spcoid)
		pending = last_tabspace_time = pgstat_prep_tablespace_time(spcoid);
...
pgstat_flush_tablespace_times(bool nowait)
{
	 ...
	 * lookups below keep a reference to each entry they flush into, but
	...
	/* the entry it points to may be removed below */
	last_tabspace_time = NULL;
