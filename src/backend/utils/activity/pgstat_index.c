 ...
 * is flushed and PGSTAT_FLUSH_DONE is returned; index stats are entirely
 * non-transactional, so there is never anything to defer and the flush is
 * always complete regardless of xact_boundary.
 *
...
pgstat_index_flush_cb(PgStat_EntryRef *entry_ref, bool nowait, bool xact_boundary)
{
	...
	/*
	 * Ignore entries that didn't accumulate any new counts since the last
	 * flush, such as indexes that were opened by the planner but not used.
	 */
