...
CopyFrom(CopyFromState cstate)
{
	...
	if (check_enable_rls(RelationGetRelid(cstate->rel), InvalidOid, false) == RLS_ENABLED)
	{
		...
		if (cstate->rel->rd_rules != NULL)
			ereport(ERROR,
					errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("cannot copy to table \"%s\"",
						   RelationGetRelationName(cstate->rel)),
					errdetail("COPY FROM is not supported on tables with rules when row-level security is enabled."),
		...
		/*
		 * Run parse analysis and rewrite.  RLS is also processed during query
		 * rewrite.
		 */
		...
		Assert(query->querySource == QSRC_ORIGINAL);
	...
	else if (queryDesc != NULL && volatile_wco)
	{
		/*
		 * Can't support multi-inserts if the RLS quals contain any volatile
		 * function expressions or any sub-selects.  Similarly to the trigger
		 * case above, such quals may query the table we're inserting into.  A
		 * sub-select can do that even without volatile functions.
		 */
		insertMethod = CIM_SINGLE;
	}
	else
