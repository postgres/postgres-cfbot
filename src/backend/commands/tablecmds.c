	...
	if (!cmdcon->is_enforced || changed)
	{
		...
		if (!recursing && !currcon->connoinherit)
		{
			if (cmdcon->is_enforced)
				...
				/*
				 * ENFORCED does not need to check parents' enforceability, so
				 * use find_all_inheritors() to tolerate potential inheritance
				 * cycles.
				 */
				...
				/*
				 * NOT ENFORCED needs to check parents' enforceability to
				 * ensure consistency.  This requires each parent to appear
				 * before its children in the list, so call
				 * find_all_inheritors_ordered(). If there is an inheritance
				 * cycle, this will fail.
				 */
