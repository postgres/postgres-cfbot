				...
				AlterTableUtilityContext *context)
{
	...
	if (RELKIND_HAS_STORAGE(relkind))
	{
		...
		has_domain_constraints = DomainHasConstraints(attribute->atttypid, NULL, NULL);
 ...
 * However, this is only valid when the constraint contains immutable built-in
 * functions. In that case, scan_only is set to true.
 */
static bool
ATColumnChangeRequiresRewrite(Node *expr, AttrNumber varattno, bool *scan_only)
{
	...
	for (;;)
	{
		...
		else if (IsA(expr, CoerceToDomain))
		{
			...
			has_constraints = DomainHasConstraints(d->resulttype, NULL, &has_mutable);
			...
			else if (has_constraints)
			{
				...
				typtup = SearchSysCache1(TYPEOID, ObjectIdGetDatum(d->resulttype));
				if (!HeapTupleIsValid(typtup))
					elog(ERROR, "cache lookup failed for type %u", d->resulttype);

				typform = (Form_pg_type) GETSTRUCT(typtup);

				if (typform->typbasetype >= FirstUnpinnedObjectId ||
					DomainConstrContainUserDefinedFunc(d->resulttype))
				{
