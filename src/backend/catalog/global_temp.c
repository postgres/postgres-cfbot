 ...
 */
static void
gtr_update_temp_frozen_xids(void)
{
	TransactionId min_relfrozenxid = InvalidTransactionId;
	MultiXactId min_relminmxid = InvalidMultiXactId;
	...
	if (gtr_local_usage)
	{
		...
		while ((entry = hash_seq_search(&status)) != NULL)
		{
			TransactionId relfrozenxid = entry->history.info.relfrozenxid;
			MultiXactId relminmxid = entry->history.info.relminmxid;
	...
	/* Store the new minimum values in our PGPROC struct */
...
TrackGlobalTempRelation(Relation relation, bool isNew)
{
	...
	if (!found)
	{
		 ...
		 * session), then schedule them to be updated on commit, to account
		 * for this relation.  If they have been set, it must have been from
		 * an earlier transaction, so this relation will not affect them.
		 */
...
ForgetGlobalTempRelation(Oid relid)
{
	...
	Assert(entry->stopped_subid == InvalidSubTransactionId);

	entry->stopped_subid = GetCurrentSubTransactionId();
	EOXactUsageListAdd(relid);

	/* Schedule tempfrozenxid and tempminmxid to be updated on commit */
	UpdateTempFrozenXids(false);
}
 ...
 *	temporary tables.
 *
 *	If "immediate" is true, the change takes place immediately, and cannot be
 *	rolled back.  This is only safe if none of the changes to any relation's
 *	frozen Xids can be rolled back, so really only VACUUM qualifies.
 *
 *	If "immediate" is false, the updates are deferred until main transaction
 *	commit.  This is required in cases where some or all of the changes made
 *	may be rolled back (e.g., a DROP that makes it seem as though
 *	tempfrozenxid and/or tempminmxid can be advanced, only to be rolled back).
 *	Other backends must only see the final state that we commit.
 */
void
UpdateTempFrozenXids(bool immediate)
{
	if (immediate)
		/* Update tempfrozenxid and tempminmxid immediately */
		gtr_update_temp_frozen_xids();
	else
		/* Flag tempfrozenxid and tempminmxid to be updated on commit */
