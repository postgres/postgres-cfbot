	...
	*binary_mode = false;
	...
	foreach(lc, params)
	{
								   ...
								   binary_mode))
		{
			...
			/*
			 * We can't make this Param a cache key (e.g. its type has no
			 * hash opclass).  Rather than abandoning Memoize altogether,
			 * record the Param ID so that create_memoize_plan can exclude
			 * it from keyparamids.  The executor will then purge the cache
			 * whenever its value changes, which is less efficient than
			 * including it as a key but still correct.
			 */
			*unhashable_params = bms_add_member(*unhashable_params,
												param->paramid);
		}
	}
				 ...
				 JoinPathExtraData *extra)
{
	...
	Bitmapset  *unhashable_params = NULL;
