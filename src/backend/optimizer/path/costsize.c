					...
					JoinPathExtraData *extra)
{
	 ...
	 * are indeed evaluated once per hashjointuples tuple, but the joinquals
	 * not used for hashing are really evaluated once per tuple passing the
	 * hash quals.  For SEMI, RIGHT_SEMI, and ANTI joins a short-circuit at
	 * the first match limits the evaluations, and we accept the imprecision.
	 *
	 * A right anti join has no such short-circuit, so its joinquals are
	 * evaluated for every tuple passing the hash quals.  Charge them on that
	 * count, and only cpu_tuple_cost on hashjointuples.  (This overcharges
