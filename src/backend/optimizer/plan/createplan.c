					 ...
					 NestPath *best_path)
{
	...
	if (IS_OUTER_JOIN(best_path->jpath.jointype))
	{
		 ...
		 * gate the inner side with them instead of re-checking them per inner
		 * row.
		 */
		foreach_node(RestrictInfo, rinfo, joinrestrictclauses)
		{
			...
			/*
			 * Gating evaluates the clause once per outer tuple instead of once
			 * per join-candidate pair.  That is only transparent if the clause
			 * yields the same answer every time. Hence, refuse volatile
			 * expressions and SubPlans whose own evaluation count would change.
			 */
	...
	if (gating_clauses != NIL)
	{
		 ...
		 * This mirrors create_gating_plan()'s logic.
		 */
