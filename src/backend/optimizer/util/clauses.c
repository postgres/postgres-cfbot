		...
		case T_SafeTypeCastExpr:
			{
				...
				/*
				 * Set up a fresh escontext while processing castexpr, so that
				 * any soft errors raised while trying to const-fold it are
				 * left unfolded for the executor to handle at runtime (see
				 * ExecEvalSafeTypeCastExpr) rather than causing a hard ERROR
				 * here.  We must save/restore the previous escontext rather
				 * than assuming it was NULL, since a SafeTypeCastExpr can be
				 * nested inside another one's castexpr; unconditionally
				 * clearing it to NULL (or leaving our own escontext behind)
				 * would corrupt the outer SafeTypeCastExpr's error-catching
				 * state for the remainder of its own castexpr processing.
				 */
				save_escontext = context->escontext;
				...
				/*
				 * defexpr is not allowed to fail softly: it's supposed to be
				 * a well-behaved default, so let any error in it propagate
				 * normally.  Do this regardless of nesting; only after that
				 * do we restore whatever escontext the caller had active.
				 */
