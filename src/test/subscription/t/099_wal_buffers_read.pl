...
# A WAL read that comes entirely from the WAL buffers must not leave a stale
# open segment behind for a later file read. Exercised for
# read_local_xlog_page() (via pg_walinspect) and the logical walsender (via
# pg_recvlogical).
...
# buffers rather than from a file, and keep the WAL writer asleep so that its
# opportunistic buffer pre-initialization does not evict that page meanwhile.
$node->append_conf(
	'postgresql.conf', qq(wal_buffers = 64MB
...
$node->safe_psql('postgres',
	"SELECT injection_points_attach('wal-read-from-buffers-force-miss',
									'notice')");
...
unlike($rerr, qr/unexpected pageaddr/,
	'walsender did not reuse a stale segment');
