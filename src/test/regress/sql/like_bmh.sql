...
CREATE FUNCTION like_bmh_plpgsql_like(s text, p text) RETURNS boolean
LANGUAGE plpgsql AS $$
BEGIN
	RETURN s LIKE p;
END
$$;
SELECT s, p, like_bmh_plpgsql_like(s, p)
FROM (VALUES ('xxabcdxx', '%abcd%'),
			 ('xxabcdxx', '%wxyz%'),
			 ('xxwxyzxx', '%wxyz%')) AS v(s, p);
SELECT like_bmh_plpgsql_like(s, p)
FROM (VALUES ('xxabcdxx', '%abcd%'),
			 ('xabcdx', '%abcd%'),
			 ('abcd', '%abcd%')) AS v(s, p);

CREATE FUNCTION like_bmh_plpgsql_like_name(s name, p name) RETURNS boolean
LANGUAGE plpgsql AS $$
BEGIN
	RETURN s LIKE p;
END
$$;
SELECT s, p, like_bmh_plpgsql_like_name(s, p)
FROM (VALUES ('xxclassxx'::name, '%class%'::name),
			 ('xxclassxx'::name, '%other%'::name),
			 ('xxotherxx'::name, '%other%'::name)) AS v(s, p);

DROP FUNCTION like_bmh_plpgsql_like(text, text);
DROP FUNCTION like_bmh_plpgsql_like_name(name, name);
