...
CREATE TABLE pgtbl_ddl_part_other.pt_c
    ...
    FOR VALUES FROM (0) TO (100);
-- The ALTER INDEX ... ATTACH PARTITION must name the child index with its
-- own schema, which is not the target table's; a bare name would resolve
-- through search_path on replay, or not at all.
CREATE INDEX pt_val_idx ON pgtbl_ddl_part_s.pt (val);
...
-- CLUSTER ON cross-kind dependency: if the clustered index is backed by a
-- constraint whose kind is filtered, CLUSTER ON is silently skipped (the
-- index won't exist on replay anyway).
CREATE TABLE clust_pk (a int PRIMARY KEY);
CLUSTER clust_pk USING clust_pk_pkey;
-- Filtering out primary_key suppresses both the PK and CLUSTER ON.
SELECT * FROM pg_get_table_ddl('clust_pk'::regclass, owner => false,
                               except_kinds => ARRAY['primary_key']);
DROP TABLE clust_pk;
-- UNIQUE-backed clustered index: filtering unique also silently skips CLUSTER ON.
CREATE TABLE clust_uniq (a int NOT NULL UNIQUE);
CLUSTER clust_uniq USING clust_uniq_a_key;
SELECT * FROM pg_get_table_ddl('clust_uniq'::regclass, owner => false,
                               except_kinds => ARRAY['unique']);
DROP TABLE clust_uniq;

-- Plain-index cluster: except_kinds=primary_key does NOT suppress CLUSTER ON
-- when the clustered index is not backed by the primary key.
CREATE TABLE clust_plain (a int PRIMARY KEY, b int);
CREATE INDEX clust_plain_b ON clust_plain (b);
CLUSTER clust_plain USING clust_plain_b;
SELECT * FROM pg_get_table_ddl('clust_plain'::regclass, owner => false,
                               except_kinds => ARRAY['primary_key']);
DROP TABLE clust_plain;

-- PK-backed CLUSTER ON with only_kinds=table,primary_key: the backing kind
-- (primary_key) is included, so CLUSTER ON must be emitted even though "index"
-- is absent from the filter.
CREATE TABLE clust_pk_only (a int PRIMARY KEY);
CLUSTER clust_pk_only USING clust_pk_only_pkey;
SELECT * FROM pg_get_table_ddl('clust_pk_only'::regclass, owner => false,
                               only_kinds => ARRAY['table','primary_key']);
DROP TABLE clust_pk_only;

-- PK-backed CLUSTER ON with except_kinds=index: the backing kind is primary_key
-- (not index), so CLUSTER ON must survive despite "index" being excluded.
CREATE TABLE clust_pk_exidx (a int PRIMARY KEY);
CLUSTER clust_pk_exidx USING clust_pk_exidx_pkey;
SELECT * FROM pg_get_table_ddl('clust_pk_exidx'::regclass, owner => false,
                               except_kinds => ARRAY['index']);
DROP TABLE clust_pk_exidx;

-- Self-referential FK cross-kind dependency: filtering out the PK/UNIQUE
-- that the FK targets raises an error (the FK would dangle on replay).
CREATE TABLE self_ref_fk (id int PRIMARY KEY, parent_id int REFERENCES self_ref_fk(id));
-- Excluding primary_key while keeping foreign_key must error.
SELECT * FROM pg_get_table_ddl('self_ref_fk'::regclass, owner => false,
                               except_kinds => ARRAY['primary_key']);
-- Excluding both resolves the conflict.
SELECT * FROM pg_get_table_ddl('self_ref_fk'::regclass, owner => false,
                               except_kinds => ARRAY['primary_key','foreign_key']);
DROP TABLE self_ref_fk;

-- UNIQUE-backed self-referential FK.
CREATE TABLE self_ref_uniq (a int NOT NULL UNIQUE, b int REFERENCES self_ref_uniq(a));
SELECT * FROM pg_get_table_ddl('self_ref_uniq'::regclass, owner => false,
                               except_kinds => ARRAY['unique']);
DROP TABLE self_ref_uniq;
-- Excluding both unique and foreign_key resolves the unique-backed conflict.
CREATE TABLE self_ref_uniq2 (a int NOT NULL UNIQUE, b int REFERENCES self_ref_uniq2(a));
SELECT * FROM pg_get_table_ddl('self_ref_uniq2'::regclass, owner => false,
                               except_kinds => ARRAY['unique','foreign_key']);
DROP TABLE self_ref_uniq2;

-- Plain unique index (no backing constraint) used as the referenced key for a
-- self-referential FK.  except_kinds=index must error because the unique index
-- would not be emitted, leaving the FK dangling.
CREATE TABLE self_ref_uidx (id int NOT NULL, parent_id int);
CREATE UNIQUE INDEX self_ref_uidx_id_uidx ON self_ref_uidx (id);
ALTER TABLE self_ref_uidx ADD CONSTRAINT self_ref_uidx_fk
    FOREIGN KEY (parent_id) REFERENCES self_ref_uidx (id);
SELECT * FROM pg_get_table_ddl('self_ref_uidx'::regclass, owner => false,
                               except_kinds => ARRAY['index']);
DROP TABLE self_ref_uidx;

-- only_kinds=foreign_key on a self-referential FK table: the backing PK is not
-- in the only set, so the function must error.
CREATE TABLE self_ref_only (id int PRIMARY KEY, parent_id int REFERENCES self_ref_only(id));
SELECT * FROM pg_get_table_ddl('self_ref_only'::regclass, owner => false,
                               only_kinds => ARRAY['foreign_key']);
DROP TABLE self_ref_only;

-- Cross-table FK: except_kinds=primary_key does NOT raise an error when the
-- FK references a different table's PK -- only self-referential FKs are checked.
CREATE TABLE xfk_tgt (id int PRIMARY KEY);
CREATE TABLE xfk_src (id int PRIMARY KEY, fk_id int REFERENCES xfk_tgt(id));
SELECT * FROM pg_get_table_ddl('xfk_src'::regclass, owner => false,
                               except_kinds => ARRAY['primary_key']);
DROP TABLE xfk_src;
DROP TABLE xfk_tgt;
...
-- Issue 11a continued: bigint column with sequence narrowed to integer.
CREATE TABLE idt2 (a bigint GENERATED ALWAYS AS IDENTITY);
ALTER SEQUENCE idt2_a_seq AS integer;
SELECT stmt FROM pg_get_table_ddl('idt2', owner => false) stmt;

-- Non-default MINVALUE combined with AS type change: MINVALUE is emitted
-- inline; the AS type goes to the post-CREATE ALTER SEQUENCE.
CREATE TABLE idt3 (a int GENERATED BY DEFAULT AS IDENTITY);
ALTER SEQUENCE idt3_a_seq AS smallint MINVALUE -100;
SELECT stmt FROM pg_get_table_ddl('idt3', owner => false) stmt;
...
-- and the other direction: an unlogged table whose sequence was SET LOGGED.
CREATE UNLOGGED TABLE idu (a int GENERATED ALWAYS AS IDENTITY);
ALTER SEQUENCE idu_a_seq SET LOGGED;
SELECT stmt FROM pg_get_table_ddl('idu', owner => false) stmt;
