...
-- INSERT SET targeting different subfields/elements of the same column
-- (these are distinct targets, not duplicate column assignments)
create type insertset_comp as (x int, y int);
create table insertsetfield (id int primary key, c insertset_comp);
create table insertsetarr (id int primary key, arr int[]);

-- Different subfields of the same composite column
insert into insertsetfield set id=1, c.x=7, c.y=8;
select * from insertsetfield order by id;

-- Different elements of the same array column
insert into insertsetarr set id=1, arr[1]=30, arr[2]=40;
select * from insertsetarr order by id;

-- A whole-column assignment still conflicts with a partial assignment of
-- the same column (should fail)
insert into insertsetfield set id=2, c=row(1,2), c.x=9;

-- Multi-row SET where rows target different elements of the same array
-- column can't be represented by the shared column list used for
-- multi-row SET (same limitation as INSERT ... (col-list) VALUES), so it
-- must be rejected rather than silently misassigning values (should fail)
insert into insertsetarr set (id=2, arr[1]=111), (id=3, arr[2]=222);
select * from insertsetarr order by id;

-- Multi-row SET where rows target the same element of the same array
-- column works fine, just like the equivalent column-list VALUES form
insert into insertsetarr set (id=4, arr[1]=1), (id=5, arr[1]=2);
select * from insertsetarr order by id;

-- A row that omits an indirection-targeted column entirely, while other
-- rows in the same statement target an element/subfield of it, can't be
-- represented either: the shared target's expression is applied uniformly
-- to every row, so this row's "don't touch this column" can't be
-- distinguished from "set this element to its default" (which is itself
-- illegal).  Must be rejected (should fail).
create table insertsetarrdef (id int primary key, arr int[] default array[1]);
insert into insertsetarrdef set (id=70, arr[1]=1), (id=71, arr[1]=2), (id=72);
select * from insertsetarrdef order by id;

-- Likewise, a row assigning the whole column (even to DEFAULT) can't be
-- mixed with another row that targets one of its elements (should fail)
insert into insertsetarrdef set (id=1, arr[1]=1), (id=2, arr=default);
select * from insertsetarrdef order by id;

drop table insertsetfield;
drop table insertsetarr;
drop table insertsetarrdef;
drop type insertset_comp;
