...
-- star expansion preserves the source column names used for matching
create table byname_src (b int, c int, a int);
insert into byname_src values (2, 3, 1);
truncate byname_t;
insert into byname_t by name select * from byname_src;
select a, b, c from byname_t order by a;

-- resjunk columns used to sort the source query are ignored
truncate byname_t;
insert into byname_t by name select a, b from byname_src order by c;
select a, b, c from byname_t order by a;
drop table byname_src;

-- A VALUES construct can be used through a query that names its columns
truncate byname_t;
insert into byname_t by name
  select * from (values (7, 8)) as src(b, a);
select a, b, c from byname_t order by a;
...
-- target relation and columns are validated before the source form
insert into byname_missing by name values (1);
insert into byname_t (missing) by name values (1);
