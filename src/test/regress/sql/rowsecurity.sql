...
SET SESSION AUTHORIZATION regress_rls_alice;
CREATE TABLE copy_t (owner text);
CREATE FUNCTION under_quota(o text) RETURNS bool LANGUAGE plpgsql VOLATILE
AS $$ BEGIN RETURN (SELECT count(*) FROM copy_t WHERE owner = o) < 2 AND; END $$;
CREATE POLICY p1 ON copy_t FOR SELECT USING (true);
CREATE POLICY p2 ON copy_t FOR INSERT WITH CHECK (under_quota(owner));
ALTER TABLE copy_t ENABLE ROW LEVEL SECURITY;
ALTER TABLE copy_t FORCE ROW LEVEL SECURITY;
GRANT SELECT, INSERT ON copy_t to regress_rls_bob;
SET SESSION AUTHORIZATION regress_rls_bob;
COPY copy_t FROM STDIN; -- error
regress_rls_bob
regress_rls_bob
regress_rls_bob
\.
SET SESSION AUTHORIZATION regress_rls_alice;
DROP TABLE copy_t;
DROP FUNCTION under_quota(text);
RESET SESSION AUTHORIZATION;
