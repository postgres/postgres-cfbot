...
-- Two window aggregate calls with different ON EMPTY defaults, same
-- underlying aggregate/args/frame (so they may share transition state):
-- each must still get its own default, not one shared between them.
SELECT id, val,
  sum(val, -111 ON EMPTY) OVER w AS d1,
  sum(val, -222 ON EMPTY) OVER w AS d2,
  sum(val) OVER w AS plain
FROM win_on_empty
WINDOW w AS (ORDER BY id ROWS BETWEEN 1 PRECEDING AND 1 PRECEDING)
ORDER BY id;

-- Frames containing only NULL values: ON EMPTY is exactly
-- COALESCE(normal_windowed_result, default_expression), so it fires
-- whenever the frame's ordinary result is NULL -- which it is here, since
-- sum/avg of an all-NULL frame is NULL regardless of frame shape.
SELECT id, val,
...
-- Same COALESCE-equivalence with a growing (RANGE UNBOUNDED PRECEDING)
-- frame instead of a sliding one: sum/avg/max are all NULL over an all-NULL
-- partition, so ON EMPTY fires for all three here.
SELECT grpid, id, val,
