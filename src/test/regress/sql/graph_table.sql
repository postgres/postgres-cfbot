...
-- Multi-pattern: two patterns joined via shared start vertex 'a'
...
-- Same variable with different label expressions
SELECT a_name, b_name, c_name FROM GRAPH_TABLE (g1
    MATCH (a IS vl1)-[]->(b IS vl2),
          (b IS vl3)-[]->(c IS vl3)
    COLUMNS (a.vname AS a_name, b.vname AS b_name, c.vname AS c_name)
...
-- Disconnected patterns: verify cross join by checking row count equals product
-- vl1->vl2 gives 3 paths; vl1->vl3 gives 5 paths (v2 also carries vl3);
-- cross product = 3 x 5 = 15 rows
SELECT count(*) FROM GRAPH_TABLE (g1
    MATCH (a IS vl1)-[]->(b IS vl2),
          (c IS vl1)-[]->(d IS vl3)
    COLUMNS (1 AS one)
);
