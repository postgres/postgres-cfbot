...
-- Check that dropping the temp schema doesn't leave an inconsistent state
\c -
BEGIN;
CREATE TEMP TABLE test_temp_t1(f1 int);
SELECT pg_my_temp_schema()::regnamespace AS mytempschema \gset
DROP SCHEMA :mytempschema CASCADE;
CREATE TEMP TABLE test_temp_t2(f1 int);
SELECT EXISTS (SELECT 1 FROM pg_namespace WHERE oid = pg_my_temp_schema())
    AS temp_schema_exists;
COMMIT;

-- the same, with the drop in a later transaction
SELECT pg_my_temp_schema()::regnamespace AS mytempschema \gset
DROP SCHEMA :mytempschema CASCADE;
CREATE TEMP TABLE test_temp_t3(f1 int);
SELECT EXISTS (SELECT 1 FROM pg_namespace WHERE oid = pg_my_temp_schema())
    AS temp_schema_exists;
