...
SELECT CAST(ARRAY[1, 'x'] AS point[] DEFAULT NULL ON CONVERSION ERROR);
SELECT CAST('\x01'::bytea::uuid AS text DEFAULT 'x1' ON CONVERSION ERROR);
SELECT CAST(('abc'::text)::int + abs(NULL) AS int DEFAULT 3 ON CONVERSION ERROR);
SELECT CAST(ARRAY[1,2] || 3 AS boolean DEFAULT false ON CONVERSION ERROR);
...
CREATE TABLE tcast(a text[], b int GENERATED BY DEFAULT AS IDENTITY, c text default 'c');
...
SELECT CAST(c COLLATE "C" AS int DEFAULT b ON CONVERSION ERROR) FROM tcast;

CREATE DOMAIN ddomain AS numeric CHECK ((1/ (VALUE - 1)) > 0);
SELECT CAST ('1' AS ddomain DEFAULT NULL ON CONVERSION ERROR); -- error
SELECT CAST (1 AS ddomain DEFAULT NULL ON CONVERSION ERROR); -- error
...
CREATE FUNCTION plus1(int) RETURNS int LANGUAGE sql IMMUTABLE RETURN $1 + 1;
EXPLAIN (VERBOSE, COSTS OFF)
SELECT plus1(CAST(x AS int DEFAULT 0 ON CONVERSION ERROR)) FROM (VALUES ('1'), ('2')) AS t(x);
...
SELECT CAST(ARRAY[['1'], ['three'],['a']] AS d_int_arr
       ...
       CAST('a'::text AS int DEFAULT 3 ON CONVERSION ERROR) AS cast3;
\sv safecastview1
