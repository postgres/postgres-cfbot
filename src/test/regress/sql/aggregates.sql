...
SELECT product(val::float8) FROM product_signs;          -- -0 (signed zero, float8 semantics)
...
-- Unlike numeric, product() over float4/float8 multiplies using the same
-- overflow-checked primitive sum() uses, so a product of finite values
-- that would exceed the range of double precision raises an error rather
-- than silently becoming Infinity.  (Infinity is only produced when it is
-- already present in the input; see the special-value tests below.)
CREATE TABLE product_floats_overflow (val float4);
INSERT INTO product_floats_overflow SELECT '3.4e38' FROM generate_series(1, 9);
SELECT product(val) FROM product_floats_overflow;           -- overflow error
SELECT product(val::float8) FROM product_floats_overflow;   -- overflow error
...
DROP TABLE product_floats_overflow;
