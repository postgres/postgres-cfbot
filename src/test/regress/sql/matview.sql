...
SELECT * FROM mvtest_unlogged ORDER BY type;
-- REFRESH CONCURRENTLY works too, and keeps the existing epoch stamp
SELECT relpopulated AS mvtest_unlogged_stamp FROM pg_class
  WHERE oid = 'mvtest_unlogged'::regclass \gset
REFRESH MATERIALIZED VIEW CONCURRENTLY mvtest_unlogged;
SELECT relpopulated = :mvtest_unlogged_stamp AS stamp_kept FROM pg_class
  WHERE oid = 'mvtest_unlogged'::regclass;
SELECT pg_matview_is_populated('mvtest_unlogged'::regclass);
SELECT * FROM mvtest_unlogged ORDER BY type;
