...
-- A planted table and domain, for the relation and type lookups.
CREATE TABLE test_overload.cfg (who text);
CREATE DOMAIN test_overload.planted_dom AS text;
RESET ROLE;
...
-- A planted relation or type is refused whether the script qualifies the
-- name with the schema or leaves it to search_path.
CREATE EXTENSION test_ext_overload_strict SCHEMA test_overload VERSION '7.0';  -- fails
CREATE EXTENSION test_ext_overload_strict SCHEMA test_overload VERSION '8.0';  -- fails
CREATE EXTENSION test_ext_overload_strict SCHEMA test_overload VERSION '9.0';  -- fails
CREATE EXTENSION test_ext_overload_strict SCHEMA test_overload VERSION '10.0';  -- fails
-- The plants are untouched.
SELECT count(*) AS rows_in_plant FROM test_overload.cfg;
