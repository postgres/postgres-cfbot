...
# An in-transaction flush forced by pg_stat_force_next_flush() must not merge
...
# The transactional counter must only reach shared memory at a transaction
# boundary.  Hold a transaction open in a background session and force an
# in-transaction flush.  The non-transactional counter becomes visible right
# away, while the transactional one stays deferred until commit.
$node->safe_psql('postgres',
...
# After the transaction boundary the deferred counter is flushed too.
