...
sub new
{
	...
	#
	# Forward hostname canonicalization is disabled too: a short-lived
	# ticket test (see the "lifetime" parameter to create_ticket() below)
	# needs the ticket's lifetime to be spent on the wire, not partly eaten
	# by a DNS lookup during ticket acquisition, which could otherwise make
	# the ticket expire before the test's timing assumptions expect it to.
	append_to_file(
		$krb5_conf,
		qq![logging]
