...
$node_standby->psql(
	...
	stderr => \$stderr);
ok( $stderr =~ /WAIT cannot be executed while the current transaction holds a snapshot/,
	"get an error when running in a transaction with an isolation level higher than REPEATABLE READ"
);
ok( $stderr =~ /isolation level higher than READ COMMITTED, so it holds a snapshot/,
...
unlike($stderr, qr/isolation level/,
