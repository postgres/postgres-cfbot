...
## Check that the histogram bucket counts are properly loaded
$node->restart;

my $restartedcount = $node->safe_psql('postgres',
	"SELECT sum(bucket_count) AS hist_bucket_count_sum FROM pg_stat_get_io_histogram() " .
	"WHERE backend_type='client backend' AND object='relation' AND context='normal'");

cmp_ok(
	$restartedcount, '>=', $countafter,
	"pg_stat_io_histogram: latency buckets loaded OK after restart");
